...
public struct PNLoader {
    ...
    public static func showLoading(_ text: String, disableUI: Bool) {
        PNLoader().startLoadingActivity(text, with: disableUI)
    }
    
    public static func showLoading() {
        PNLoader().startLoadingActivity("", with: false)
    }
...
fileprivate extension PNLoader{
    ...
    fileprivate func startLoadingActivity(_ text: String,with disableUI: Bool){
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) {
            ...
            PNLoader.instance = LoadingResource(text: text, disableUI: disableUI)
            DispatchQueue.main.async {
                if PNLoader.viewBackgroundDark {
                    if PNLoader.backgroundView == nil {
                        PNLoader.backgroundView = UIView(frame: UIApplication.shared.keyWindow!.frame)
                    }
                    PNLoader.backgroundView.backgroundColor = UIColor.black.withAlphaComponent(0)
                    topMostViewController?.view.addSubview(PNLoader.backgroundView)
                    UIView.animate(withDuration: 0.2, animations: {PNLoader.backgroundView.backgroundColor = PNLoader.backgroundView.backgroundColor?.withAlphaComponent(0.5)})
                }
                PNLoader.instance?.showLoadingActivity()
            }
        }
    }
}
